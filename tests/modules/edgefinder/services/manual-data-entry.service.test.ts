...
describe('ingestManualEntry', () => {
  ...
  it('rate decision, first release: levels stored, previousValue null, first_release flagged', async () => {
    mockedFindUnique.mockResolvedValue({
      id: 'ind-fed',
      code: 'US_FED_RATE',
    ...
    // The submitted forecast (3.4, from baseInput()'s default) is stored as
    // the level it is. It used to be dropped here: converting it needed a
    // prior rate, and a first release has none — so a first decision could
    // never be scored even with an expectation on file.
    expect(result.forecastValue).toBe(3.4);

    const call = mockedUpsert.mock.calls[0][0];
    expect(call.value).toBe(5.0);
    expect(call.forecastValue).toBe(3.4);
    // No prior decision on file, so no prior rate — the submitted `previous`
    // does not stand in for it.
    expect(call.previousValue).toBeNull();
    const meta = call.sourceMetadata as Record<string, unknown>;
    // The levels live in the columns now, not in metadata.
    expect(meta.rate_level).toBeUndefined();
    expect(meta.first_release).toBe(true);
    expect(meta.manualEntry).toBe(true);
  });

  it('rate decision with prior 5.00 → new 5.25: stores the levels, not a delta', async () => {
    mockedFindUnique.mockResolvedValue({
      id: 'ind-fed',
      code: 'US_FED_RATE',
      name: 'US Fed Funds Rate',
    ...
    // The rate this decision moved from, taken from the last one on file.
  ...
  it('rate decision with prior 5.50 → new 5.25 (a cut): stores the levels', async () => {
    mockedFindUnique.mockResolvedValue({
      id: 'ind-fed',
      code: 'US_FED_RATE',
      name: 'US Fed Funds Rate',
    ...
    mockedFindFirst.mockResolvedValue({ value: 5.5 });
    ...
    expect(result.value).toBe(5.25);
    const call = mockedUpsert.mock.calls[0][0];
    expect(call.previousValue).toBe(5.5);
  });

  // The expected rate is stored as the level it is, in the same column and the
  // same unit every other indicator uses for its forecast.
  describe('rate decision forecast (expected rate) persistence', () => {
    it('expected rate submitted → forecastValue stores that level', async () => {
      mockedFindUnique.mockResolvedValue({
        id: 'ind-fed',
        code: 'US_FED_RATE',
        name: 'US Fed Funds Rate',
      ...
      expect(result.value).toBe(5.25);
      expect(result.forecastValue).toBe(5.25);
      expect(result.rateExpectedLevel).toBe(5.25);

      const call = mockedUpsert.mock.calls[0][0];
      expect(call.forecastValue).toBe(5.25);
      const meta = call.sourceMetadata as Record<string, unknown>;
      // Levels are columns now; metadata carries none of them.
      expect(meta.expected_rate_level).toBeUndefined();
    });

    it('a surprise hike (actual 5.50, expected 5.25) stores both levels, and the handler diffs them', async () => {
      mockedFindUnique.mockResolvedValue({
        id: 'ind-fed',
        code: 'US_FED_RATE',
        name: 'US Fed Funds Rate',
      ...
      expect(result.forecastValue).toBe(5.25);
      // 5.50 announced against 5.25 expected: the handler reads a +25bp
      // surprise off these two levels, which is exactly what the stored bps
      // deltas used to encode.
    });

    it('no forecast submitted (null) → forecastValue stays null', async () => {
      mockedFindUnique.mockResolvedValue({
        id: 'ind-fed',
        code: 'US_FED_RATE',
        name: 'US Fed Funds Rate',
